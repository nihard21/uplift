...
export default function UpLiftJournal() {
  ...
  useEffect(() => {
    ...
    const existingEntry = journalEntries.find(entry => 
      entry.timestamp.toDateString() === todayKey
    )
...

{
  ...
  const getBasicAnalysis = (content: string) => {
    ...
    const contentAnalysis = {
      // Work/Professional
      work: {
        keywords: ['work', 'job', 'office', 'meeting', 'deadline', 'boss', 'colleague', 'project', 'career', 'promotion', 'stress', 'pressure', 'exhausted', 'tired', 'overwhelmed'],
      ...
      // Relationships
      relationships: {
        keywords: ['friend', 'family', 'partner', 'relationship', 'love', 'breakup', 'marriage', 'dating', 'conflict', 'argument', 'support', 'lonely', 'miss', 'care', 'hurt'],
      ...
      // Health/Wellness
      health: {
        keywords: ['health', 'sick', 'pain', 'exercise', 'diet', 'sleep', 'energy', 'fatigue', 'stress', 'anxiety', 'depression', 'mood', 'wellness', 'self-care'],
      ...
      growth: {
        keywords: ['goal', 'dream', 'future', 'plan', 'achieve', 'success', 'failure', 'learn', 'grow', 'change', 'improve', 'motivated', 'inspired', 'hopeful', 'ambitious'],
    ...
    if (bestMatch && highestScore >= 2) {
      ...
      } else if (text.includes('sad') || text.includes('angry') || text.includes('frustrated') || text.includes('overwhelmed')) {
      ...
      const emotionKeywords = {
        positive: ['happy', 'joy', 'excited', 'great', 'wonderful', 'amazing', 'blessed', 'grateful', 'thankful', 'content', 'peaceful', 'energized', 'inspired', 'motivated', 'confident', 'proud', 'accomplished', 'fulfilled', 'optimistic', 'hopeful'],
        negative: ['sad', 'depressed', 'down', 'lonely', 'angry', 'frustrated', 'mad', 'irritated', 'anxious', 'worried', 'stress', 'nervous', 'scared', 'fearful', 'overwhelmed', 'exhausted', 'tired', 'hopeless', 'helpless', 'confused', 'lost', 'uncertain'],
        reflective: ['thoughtful', 'contemplative', 'mindful', 'aware', 'conscious', 'introspective', 'analytical', 'curious', 'questioning', 'searching', 'exploring', 'learning', 'growing', 'evolving', 'changing', 'transitioning', 'adapting', 'processing', 'understanding', 'realizing']
      }
  ...
  const getTrendAnalysis = (): TrendAnalysis => {
    ...
    const avgSentiment = recentEntries.reduce((sum, entry) => 
      sum + (entry.aiAnalysis?.sentimentScore || 0.5), 0
    ...
    const monthlyTrends = journalEntries.length > 7 
            ...
            >
              ...
              {entry.aiAnalysis && (
                <div className="flex items-center mt-2">
                  ...
                  <span className="text-xs text-green-400">AI Analyzed</span>
                </div>
              )}
            </Card>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {showTrends ? (
            ...
            <div className="space-y-6">
              <Card className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-blue-500/30 backdrop-blur-sm p-6">
                <div className="flex items-center mb-4">
                  <TrendingUp className="w-6 h-6 text-blue-400 mr-2" />
                  <h3 className="text-xl font-semibold text-white">Overall Mood</h3>
                </div>
                <p className="text-white/90 text-lg mb-2">{trends.overallMood}</p>
              ...
              <Card className="bg-gradient-to-r from-green-600/20 to-teal-600/20 border-green-500/30 backdrop-blur-sm p-6">
                <h3 className="text-xl font-semibold text-white mb-4">Weekly Insights</h3>
                <p className="text-white/80 leading-relaxed">{trends.weeklyInsights}</p>
              </Card>

              <Card className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 border-purple-500/30 backdrop-blur-sm p-6">
                <h3 className="text-xl font-semibold text-white mb-4">Monthly Trends</h3>
                <p className="text-white/80 leading-relaxed">{trends.monthlyTrends}</p>
              </Card>

              <Card className="bg-gradient-to-r from-orange-600/20 to-red-600/20 border-orange-500/30 backdrop-blur-sm p-6">
                <div className="flex items-center mb-4">
              ...
              {!currentEntry ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <p className="text-white/80 mb-6 text-lg">Your daily entry will be created automatically</p>
                ...
                <div className="space-y-6">
                  ...
                  {currentEntry.aiAnalysis && (
                    <Card className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-blue-500/30 backdrop-blur-sm p-6">
                      ...
                      {/* Feelings */}
                      <div className="mb-4">
                        <h4 className="text-white/90 font-medium mb-2">Overall Feeling</h4>
                        <p className="text-white/80 leading-relaxed">{currentEntry.aiAnalysis.feelings}</p>
                      </div>

                      {/* Observations */}
                      <div className="mb-4">
                        <h4 className="text-white/90 font-medium mb-2">Observations</h4>
                        <p className="text-white/80 leading-relaxed">{currentEntry.aiAnalysis.observations}</p>
                      </div>
                  ...
                  {isAnalyzing && (
                    <Card className="bg-gradient-to-r from-orange-600/20 to-red-600/20 border-orange-500/30 backdrop-blur-sm p-6">
                      <div className="flex items-center justify-center">
                        ...
                        <span className="text-white/90 text-lg">AI is analyzing your entry...</span>
                      </div>
                    </Card>
                  )}
                </div>
